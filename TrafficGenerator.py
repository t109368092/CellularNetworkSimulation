...
class GenerateCBR: #1.99Mbps
    ...
    def __init__(self, event_details, event_manager, tg_id, ue_id):
        ...
        self._lambda = 0.162 * NetworkSettings.data_amount #事件發生頻率 ms
        self.tg_id = tg_id
        self.ue_id = ue_id
    ...
    def execute(self):
        ...
        if len(NetworkSettings.ue_to_bs_mapping_table[self.ue_id]) == 2: #單一ue複數bs
            split_ratio = self.flow_control_method_dict[NetworkSettings.flow_control_method].current_split_ratio(self.ue_id)
...
class GenerateVoice: # G.729
    ...
    def __init__(self, event_details, event_manager, tg_id, ue_id):
        self.event_details = event_details
        self.event_manager = event_manager
        self._lambda = 0.1 * NetworkSettings.data_amount #到達率
        self.tg_id = tg_id
        self.ue_id = ue_id
...
class GenerateVideo: #以 YouTube 720p HD 視頻為標準
    ...
    def __init__(self, event_details, event_manager, tg_id, ue_id):
        self.event_details = event_details
        self.event_manager = event_manager
        self._lambda = 0.3125 * NetworkSettings.data_amount #傳輸到達率 ms
        self.tg_id = tg_id
        self.ue_id = ue_id
    ...
    def generate_first_event(self):
        if self.tg_id == self.ue_id + "CBR": #CBR
            event = {
                "event_target": self.tg_id,
                "event_name": "generate_CBR",
                "event_trigger_time": random.randint(1,20),
                "event_details": {}
            }
